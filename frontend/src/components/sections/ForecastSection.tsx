      ...
      <div className="section-wide">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          {scenarios.map((s) => {
            ...
            return (
              ...
              >
                <div className="font-[family-name:var(--font-display)] text-xs tracking-[0.12em] uppercase text-text-secondary mb-1">
                  {s.label}
                </div>
                <div
                ...
                {s.sub && (
                  <div
                    className="number-display text-xs mt-1"
        ...
        <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
          <div className="flex items-center gap-1">
            {SMA_CONFIGS.map((cfg) => (
              <button
                key={cfg.key}
                className={`px-3 py-1 text-xs font-semibold transition-all duration-200 font-[family-name:var(--font-mono)] ${
                  showSMA[cfg.key]
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-text-secondary font-[family-name:var(--font-mono)]">RANGE:</span>
            <input
              type="date"
              value={dateFrom}
              onChange={(e) => setDateFrom(e.target.value)}
              className="h-7 bg-surface border border-border text-text-primary text-xs px-2 focus:outline-none focus:ring-1 focus:ring-accent/30 font-[family-name:var(--font-mono)]"
            />
            ...
            {(dateFrom || dateTo) && (
              <button
                className="text-xs text-accent hover:text-accent-hover font-[family-name:var(--font-mono)]"
        ...
        <div className="mt-3 mb-4">
          <div className="flex items-center gap-1 flex-wrap">
            <span className="text-[11px] text-text-secondary font-[family-name:var(--font-display)] tracking-[0.12em] uppercase mr-2">Events:</span>
            {events.map((event) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1 px-2 py-0.5 text-[11px] transition-all duration-200 cursor-pointer whitespace-nowrap ${
                    event.visible
                      ? 'text-text-primary'
