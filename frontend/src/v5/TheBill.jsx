...
export default class TheBill extends React.Component {
  ...
  stepVault(dt, P, t) {
    ...
    if (!mobile) {
      ...
      ctx.font = '500 13px "IBM Plex Mono", monospace'; ctx.fillStyle = 'rgba(247,245,240,.55)'; ctx.fillText(new Date(T[tIdx][0] + 'T00:00:00Z').toLocaleDateString('en-GB', { month: 'short', year: 'numeric', timeZone: 'UTC' }).toUpperCase() + ' · WAS $' + (T[0][1] / 1e6).toFixed(2) + 'TN IN ' + new Date(T[0][0] + 'T00:00:00Z').toLocaleDateString('en-GB', { month: 'short', timeZone: 'UTC' }).toUpperCase(), tx, cageBot + 20);
