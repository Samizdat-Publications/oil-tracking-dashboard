...
export default class TheBill extends React.Component {
  ...
  setupBill() {
    ...
    this.vOut = E.slice(1).map((p, i) => ({ date: p[0], n: Math.round(E[i][2] - p[2]) }));
  ...
  stepCrowd(dt, P, t) {
    ...
    if (!mobile) {
      ...
      ctx.font = '500 13px "IBM Plex Mono", monospace'; ctx.fillStyle = 'rgba(247,245,240,.7)'; ctx.fillText('YOUR RAISE, AFTER PRICES · YEAR TO JULY 2026', px, py - 10);
  ...
  stepWar(dt, P, t) {
    ...
    let bx = row(0, String(Math.round(lit)), 'DEAD', 'US SERVICE MEMBERS KILLED', 'NBC NEWS · 28 AUG 2026 · INJURED: HUNDREDS', pStars);
    ...
    bx = row(1, String(Math.round(shown)), 'AIRCRAFT', 'LOST OR DAMAGED · ' + manned + ' MANNED · ' + drones + ' DRONES', 'CRS · 13 MAY 2026', pPlanes);
    ...
    bx = row(2, '$' + money.toFixed(1) + 'bn', 'SPENT', 'PENTAGON COST · TO 21 JUL 2026', 'HE SAID FOUR TO FIVE WEEKS', pMoney);
  ...
  stepVault(dt, P, t) {
    ...
    // the cage: bars in rows, filled bottom-up
    const cageW = mobile ? W - pad * 2 : (W - pad * 2) * 0.62, cageTop = Math.max(H * 0.14, 112), cageBot = H * (mobile ? 0.5 : 0.48);
    const cols = mobile ? 60 : 100, bw = cageW / cols, rows = Math.ceil(this.vTotal / cols), bh = (cageBot - cageTop) / rows;
    while (this.vMonthDone < mIdx - 1) { this.vMonthDone++; const n = this.vOut[this.vMonthDone].n; for (let i = 0; i < n; i++) this.vGone.push({ born: t + i * 12, m: this.vMonthDone }); }
    const gone = this.vGone.length, remaining = this.vTotal - gone;
    const ingot = (x, y, w, hh, a) => { ctx.globalAlpha = a; const i = Math.min(1.5, w * 0.14); ctx.fillStyle = '#B8871A'; ctx.beginPath(); ctx.moveTo(x, y + hh); ctx.lineTo(x + i, y); ctx.lineTo(x + w - i, y); ctx.lineTo(x + w, y + hh); ctx.closePath(); ctx.fill(); ctx.fillStyle = '#F2C94C'; ctx.fillRect(x + i, y, Math.max(1, w - 2 * i), Math.max(1, hh * 0.32)); ctx.globalAlpha = 1; };
    ctx.strokeStyle = 'rgba(247,245,240,.2)'; ctx.lineWidth = 1; ctx.strokeRect(pad, cageTop, cageW, cageBot - cageTop);
    for (let x = 1; x < 12; x++) { ctx.beginPath(); ctx.moveTo(pad + cageW * x / 12, cageTop); ctx.lineTo(pad + cageW * x / 12, cageBot); ctx.stroke(); }
    for (let i = 0; i < this.vTotal; i++) {
      const col = i % cols, row = Math.floor(i / cols), x = pad + col * bw, y = cageBot - (row + 1) * bh;
      const gi = i - remaining;   // bars leave from the top of the stack
      if (gi >= 0) {
        const g = this.vGone[gone - 1 - gi]; const age = Math.max(0, (t - g.born) / 700); if (age > 1.2) continue;
        const e = Math.min(1, age); ingot(x + e * 260 + 1, y + 1 - e * 40, Math.max(1, bw - 2), Math.max(1, bh - 1.5), 1 - e); continue;
      }
      ingot(x + 0.5, y + 0.5, Math.max(1, bw - 1), Math.max(1, bh - 1.5), 1);
    }
    ctx.font = '500 13px "IBM Plex Mono", monospace'; ctx.fillStyle = 'rgba(247,245,240,.7)'; ctx.textAlign = 'left';
    ctx.fillText('FOREIGN GOLD IN THE VAULT · ' + remaining.toLocaleString() + ' TONNES', pad, cageTop - 12);
    // beside it: Treasuries in Fed custody, a shrinking stack
    if (!mobile) {
      const T = B.gold.treasuries, tx = pad + cageW + 48, tw = W - pad - tx, tIdx = Math.min(T.length - 1, Math.floor(tl * T.length));
      const vmax = Math.max(...T.map(p => p[1])), cur = T[tIdx][1];
      ctx.fillStyle = 'rgba(247,245,240,.7)'; ctx.fillText('TREASURIES HELD FOR FOREIGN OFFICIALS', tx, cageTop - 12);
      const hgt = (cageBot - cageTop) * (cur / vmax) * 0.96;
      ctx.fillStyle = 'rgba(108,140,213,.25)'; ctx.fillRect(tx, cageBot - (cageBot - cageTop) * 0.96, tw, (cageBot - cageTop) * 0.96);
      ctx.fillStyle = '#6C8CD5'; ctx.fillRect(tx, cageBot - hgt, tw, hgt);
      for (let y = cageBot; y > cageBot - hgt; y -= 8) { ctx.fillStyle = 'rgba(11,30,63,.35)'; ctx.fillRect(tx, y - 1, tw, 1); }
      ctx.fillStyle = '#F7F5F0'; ctx.font = '700 30px "Barlow Condensed", sans-serif'; ctx.fillText('$' + (cur / 1e6).toFixed(2) + 'tn', tx + 10, cageBot - hgt + 34);
      ctx.font = '500 13px "IBM Plex Mono", monospace'; ctx.fillStyle = 'rgba(247,245,240,.55)'; ctx.fillText(new Date(T[tIdx][0] + 'T00:00:00Z').toLocaleDateString('en-GB', { month: 'short', year: 'numeric', timeZone: 'UTC' }).toUpperCase() + ' · WAS $2.78TN IN JAN', tx, cageBot + 20);
      // the Fed's counter-argument, in blue, at full size
      if (tl >= 1) {
        ctx.fillStyle = '#6C8CD5'; ctx.font = '700 20px "Barlow Condensed", sans-serif';
        ['THE FED\u2019S ANSWER: GOLD IS DOWN A FIFTH', 'FROM ITS JANUARY RECORD, AND THE DOLLAR', 'IS UP SINCE THE WAR BEGAN.'].forEach((l, i) => ctx.fillText(l, tx, cageBot + 44 + i * 22));
        ctx.font = '500 14px "IBM Plex Mono", monospace'; ctx.fillStyle = 'rgba(108,140,213,.85)';
        ctx.fillText('FEDS NOTES · COLIN WEISS · 3 SEP 2026', tx, cageBot + 112);
      }
    }
    ...
    const set = (ref, v) => { const el = ref.current; if (el && el.textContent !== v) el.textContent = v; };
    set(this.vaultNumRef, String(Math.round(B.gold.earmarked[0][2] - B.gold.earmarked[mIdx][2])));
    const d = mIdx === 0 ? B.gold.earmarked[0][0] : this.vOut[mIdx - 1].date;
    set(this.vaultDateRef, new Date(d + 'T00:00:00Z').toLocaleDateString('en-GB', { month: 'long', year: 'numeric', timeZone: 'UTC' }).toUpperCase() + (mIdx > 0 ? ' · −' + this.vOut[mIdx - 1].n + ' T THIS MONTH' : ''));
  }
  ...
  buildBoard(keepPhase) {
    ...
    const rowsRaw = [{ key: 'diesel', name: 'Diesel', unit: 'per gallon, EIA weekly', start: P.diesel.handover.value, end: P.diesel.latest.value, u: 'usd', series: P.diesel.fred_id + ' · 20 Jan 2025 → 31 Aug 2026' }]
      .concat(items.map(i => {
        let start = i.start, end = i.end, unit = i.note.split('.')[0], series = i.fred_id + ' · Jan 2025 → Jul 2026';
        if (sel && sel.gas && i.key === 'gasoline_ap') { start = sel.gas.handover.value; end = sel.gas.latest.value; unit = 'per gallon, ' + sel.gas.name + ', EIA weekly'; series = 'EIA ' + sel.gas.name + ' · 20 Jan 2025 → 31 Aug 2026'; }
        if (sel && sel.elecSeries && i.key === 'electricity') { start = sel.elecSeries.handover.value; end = sel.elecSeries.latest.value; unit = 'per kWh, ' + sel.name + ', EIA monthly'; series = 'EIA residential ' + sel.name + ' · Jan 2025 → Jun 2026'; }
  ...
  stepPrices(dt, P4) {
    ...
    const mon = P4 < 0.04 ? 'JANUARY 2025' : P4 > 0.7 ? 'NOW · JULY / AUGUST 2026' : new Date(b[0] + 'T00:00:00Z').toLocaleDateString('en-GB', { month: 'long', year: 'numeric', timeZone: 'UTC' }).toUpperCase();
  ...
  loop(t) {
    ...
    const run = (sec, key, dur, fn, loop) => {
      ...
      if (sec.__cue) sec.__cue.style.opacity = P >= 1 ? '1' : '0';
    };
  ...
  render() {
    ...
    return (
<div style={{fontFamily: "'Source Serif 4',Georgia,serif", background: "#0B1E3F", color: "#F7F5F0", overflow: "clip"}}>
  ...
  <section style={{background: "#F7F5F0", color: "#0B1E3F", padding: "56px 36px 72px"}}>
    <details style={{maxWidth: "820px", margin: "0 auto"}}>
      ...
      <div style={{paddingTop: "28px", display: "flex", flexDirection: "column", gap: "22px", fontSize: "17px", lineHeight: "1.5"}}>
        ...
        <p style={{margin: "0", fontSize: "15px", lineHeight: "1.55", color: "rgba(11,30,63,.8)", textWrap: "pretty"}}>"Before the war" is the mean of daily counts from 1 January 2025 to 27 February 2026 (423 days). "Last 7 days" is the trailing mean ending 30 August 2026. Before the strike every strait is held at its pre-war mean. From 28 February, Hormuz follows its daily series day by day. For the other five straits the snapshot carries daily counts from 3 May 2026; between the strike and that date the globe draws a straight line from the pre-war mean to the first observed week, and says so here. The routes the dots follow are drawn schematically through each strait; ship positions are a model, the counts are not. "The long way round Africa" rests on Cape of Good Hope traffic at 103% of its pre-war mean while Bab el-Mandeb sits at 78%.</p>
  ...
  <section data-screen-label="02 Oil doubled" style={{position: "relative", height: "100vh", scrollSnapAlign: "start"}}>
    <div style={{position: "sticky", top: "0", height: "100vh", overflow: "hidden", background: "#0B1E3F"}}>
      <div data-cue="1" style={{position: "absolute", left: "50%", bottom: "14px", transform: "translateX(-50%)", fontFamily: "'IBM Plex Mono',monospace", fontSize: "11px", letterSpacing: ".2em", color: "rgba(247,245,240,.5)", pointerEvents: "none", opacity: "0", transition: "opacity .5s", zIndex: "2"}}>SCROLL</div>
      <canvas ref={seisRef} style={{position: "absolute", inset: "0", width: "100%", height: "100%", display: "block"}}></canvas>
      <div className="g-top" style={{position: "absolute", top: "0", left: "0", right: "0", display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "28px 36px", pointerEvents: "none"}}>
      ...
      <div className="g-bottom" style={{position: "absolute", left: "0", right: "0", bottom: "0", padding: "0 36px 36px", display: "flex", flexDirection: "column", gap: "2px", pointerEvents: "none"}}>
        ...
        <p className="g-sentence" style={{margin: "clamp(8px,2vh,18px) 0 0", fontSize: "clamp(16px,2.7vh,24px)", lineHeight: "1.35", maxWidth: "640px", textWrap: "pretty", color: "#F7F5F0"}}>Crude was $57 in January. Five weeks after his strike it was $115. It fell to $70 when he agreed a ceasefire, and it is $91 now that he has broken it.</p>
      </div>
    </div>
  </section>
  ...
  <section data-screen-label="03 The strait" style={{position: "relative", height: "100vh", scrollSnapAlign: "start"}}>
    <div style={{position: "sticky", top: "0", height: "100vh", overflow: "hidden", background: "#0B1E3F"}}>
      <div data-cue="1" style={{position: "absolute", left: "50%", bottom: "14px", transform: "translateX(-50%)", fontFamily: "'IBM Plex Mono',monospace", fontSize: "11px", letterSpacing: ".2em", color: "rgba(247,245,240,.5)", pointerEvents: "none", opacity: "0", transition: "opacity .5s", zIndex: "2"}}>SCROLL</div>
      <canvas ref={straitRef} style={{position: "absolute", inset: "0", width: "100%", height: "100%", display: "block"}}></canvas>
    ...
    <div style={{position: "sticky", top: "0", height: "100vh", overflow: "hidden", background: "#0B1E3F", display: "flex", flexDirection: "column"}}>
      ...
      <div className="p-grid" style={{flex: "1", minHeight: "0", display: "grid", gridTemplateColumns: "minmax(0,1.25fr) minmax(0,1fr)", gap: "0 48px", padding: "18px 36px 0", alignItems: "stretch"}}>
        ...
        <div className="p-pump" style={{display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "flex-start", gap: "20px", minHeight: "0"}}>
          ...
          <p className="g-sentence p-sentence" style={{margin: "0", fontSize: "24px", lineHeight: "1.35", maxWidth: "520px", textWrap: "pretty", color: "#F7F5F0"}}>Fourteen things you buy, January 2025 against now. Diesel has not cost this much since 2022. Four went down; we show those too. Eggs fell because the avian flu outbreak ended, not because of anything his administration did.</p>
        </div>
      </div>
  ...
  <section style={{background: "#F7F5F0", color: "#0B1E3F", padding: "56px 36px 72px"}}>
    <details style={{maxWidth: "820px", margin: "0 auto"}}>
      ...
      <div style={{paddingTop: "28px", display: "flex", flexDirection: "column", gap: "22px", fontSize: "17px", lineHeight: "1.5"}}>
        <p style={{margin: "0", textWrap: "pretty"}}>Thirteen of the fourteen prices are the Bureau of Labor Statistics' average prices for US cities, January 2025 against the latest month, July 2026. Diesel is the Energy Information Administration's weekly national average, 20 January 2025 (the week he took office) against 31 August 2026. Each board row flips from the first price to the second; nothing in between is shown. The pump rolls through every published week.</p>
  ...
  <section data-screen-label="05 Nobody is hiring" style={{position: "relative", height: "100vh", scrollSnapAlign: "start"}}>
    <div style={{position: "sticky", top: "0", height: "100vh", overflow: "hidden", background: "#0B1E3F"}}>
      <div data-cue="1" style={{position: "absolute", left: "50%", bottom: "14px", transform: "translateX(-50%)", fontFamily: "'IBM Plex Mono',monospace", fontSize: "11px", letterSpacing: ".2em", color: "rgba(247,245,240,.5)", pointerEvents: "none", opacity: "0", transition: "opacity .5s", zIndex: "2"}}>SCROLL</div>
      <canvas ref={crowdRef} style={{position: "absolute", inset: "0", width: "100%", height: "100%", display: "block"}}></canvas>
      <div className="g-top" style={{position: "absolute", top: "0", left: "0", right: "0", display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "28px 36px", pointerEvents: "none"}}>
      ...
      <div className="g-bottom" style={{position: "absolute", left: "0", right: "0", bottom: "0", padding: "0 36px 36px", display: "flex", flexDirection: "column", gap: "2px", pointerEvents: "none"}}>
        ...
        <p className="g-sentence" style={{margin: "clamp(8px,2vh,18px) 0 0", fontSize: "clamp(16px,2.7vh,24px)", lineHeight: "1.35", maxWidth: "680px", textWrap: "pretty", color: "#F7F5F0"}}>Few people are being fired, but if you lose your job you stay out longer than at any point since he took office, and your paycheck buys less than it did a year ago.</p>
      </div>
    </div>
  </section>
  ...
  <section data-screen-label="06 What the war cost" style={{position: "relative", height: "100vh", scrollSnapAlign: "start"}}>
    <div style={{position: "sticky", top: "0", height: "100vh", overflow: "hidden", background: "#6E1B27"}}>
      <div data-cue="1" style={{position: "absolute", left: "50%", bottom: "14px", transform: "translateX(-50%)", fontFamily: "'IBM Plex Mono',monospace", fontSize: "11px", letterSpacing: ".2em", color: "rgba(247,245,240,.5)", pointerEvents: "none", opacity: "0", transition: "opacity .5s", zIndex: "2"}}>SCROLL</div>
      <canvas ref={warRef} style={{position: "absolute", inset: "0", width: "100%", height: "100%", display: "block"}}></canvas>
      <div className="g-top" style={{position: "absolute", top: "0", left: "0", right: "0", display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "28px 36px", pointerEvents: "none"}}>
      ...
      <div className="g-bottom" style={{position: "absolute", left: "0", right: "0", bottom: "0", padding: "0 36px 36px", display: "flex", flexDirection: "column", gap: "2px", pointerEvents: "none"}}>
        <div style={{fontFamily: "'Barlow Condensed',sans-serif", fontWeight: "600", fontSize: "clamp(15px,2.2vh,21px)", letterSpacing: ".12em", textTransform: "uppercase", color: "#F7F5F0", marginBottom: "clamp(4px,1vh,10px)", display: "flex", alignItems: "center", gap: "10px"}}><span style={{display: "inline-block", width: "9px", height: "9px", background: "#F7F5F0", flex: "none"}}></span>SEVEN MONTHS IN. HE SAID FIVE WEEKS.</div>
        <p className="g-sentence" style={{margin: "0", fontSize: "clamp(16px,2.7vh,24px)", lineHeight: "1.35", maxWidth: "760px", textWrap: "pretty", color: "#F7F5F0"}}>He said four to five weeks. Seven months on, the Navy resupplies from 2,200 miles away, and by one estimate a third of the missile interceptors are left. His Secretary of Defense disputes that number. His own budget request asks for $21 billion to replace them.</p>
      </div>
    </div>
  </section>
  ...
  <section style={{background: "#F7F5F0", color: "#0B1E3F", padding: "56px 36px 72px"}}>
    <details style={{maxWidth: "820px", margin: "0 auto"}}>
      ...
      <div style={{paddingTop: "28px", display: "flex", flexDirection: "column", gap: "22px", fontSize: "17px", lineHeight: "1.5"}}>
        <p style={{margin: "0", textWrap: "pretty"}}><strong style={{fontWeight: "600"}}>Eighteen stars.</strong> US service members killed, NBC News data desk count as of 28 August 2026. A Wikipedia tally of contemporaneous reporting gives 20 killed and 762 injured; the lower, sourced figure is used. Injured: "hundreds."</p>
        ...
        <p style={{margin: "0", textWrap: "pretty"}}><strong style={{fontWeight: "600"}}>$37.5 billion.</strong> Defense Secretary Hegseth to the Senate Appropriations Committee, 21 July 2026, up from $29bn on 12 May. The bar then extends by $67.1bn, the FY2026 supplemental request, of which $21bn is munitions to rebuild stockpiles. Iran struck the Navy's Bahrain logistics hub on the first day; 228 structures were damaged and the fleet resupplies at sea from Diego Garcia, 2,200 miles away.</p>
        ...
        <p style={{margin: "0", fontFamily: "'IBM Plex Mono',monospace", fontSize: "12px", letterSpacing: ".04em", color: "rgba(11,30,63,.7)", lineHeight: "1.7"}}>Sources: Defense News, 21 Jul 2026 · CRS IN12692 · NBC News data desk · Washington Post, Stars and Stripes on Bahrain · CSIS, CNN on munitions.</p>
      </div>
    </details>
  </section>
  ...
  <section data-screen-label="07 What it buys" style={{position: "relative", height: "100vh", scrollSnapAlign: "start"}}>
    <div style={{position: "sticky", top: "0", height: "100vh", overflow: "hidden", background: "#0B1E3F"}}>
      <div data-cue="1" style={{position: "absolute", left: "50%", bottom: "14px", transform: "translateX(-50%)", fontFamily: "'IBM Plex Mono',monospace", fontSize: "11px", letterSpacing: ".2em", color: "rgba(247,245,240,.5)", pointerEvents: "none", opacity: "0", transition: "opacity .5s", zIndex: "2"}}>SCROLL</div>
      <canvas ref={buyRef} style={{position: "absolute", inset: "0", width: "100%", height: "100%", display: "block"}}></canvas>
      <div className="g-top" style={{position: "absolute", top: "0", left: "0", right: "0", display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "28px 36px", pointerEvents: "none"}}>
      ...
      <div className="g-bottom" style={{position: "absolute", left: "0", right: "0", bottom: "0", padding: "0 36px 36px", display: "flex", flexDirection: "column", gap: "2px", pointerEvents: "none"}}>
        ...
        <p className="g-sentence" style={{margin: "clamp(8px,2vh,18px) 0 0", fontSize: "clamp(16px,2.7vh,24px)", lineHeight: "1.35", maxWidth: "760px", textWrap: "pretty", color: "#F7F5F0"}}>Forty-two aircraft lost or damaged, at the Pentagon's own estimate of $2.6 billion: that is {buyPS5} million PlayStation 5s, or {buyDays} days of every gallon of diesel the country burns, or {buyDogs} Costco hot dogs for every American. The war itself has cost {buyRatio} times that, and he has asked Congress for $67.1 billion more.</p>
      </div>
    </div>
  </section>
  ...
  <section style={{background: "#F7F5F0", color: "#0B1E3F", padding: "56px 36px 72px"}}>
    <details style={{maxWidth: "820px", margin: "0 auto"}}>
      ...
      <div style={{paddingTop: "28px", display: "flex", flexDirection: "column", gap: "22px", fontSize: "17px", lineHeight: "1.5"}}>
        ...
        <p style={{margin: "0", fontSize: "15px", lineHeight: "1.55", color: "rgba(11,30,63,.8)", textWrap: "pretty"}}>"One for every {buyHH} households" is {buyPS5} million consoles against about 132 million US households. "{buyDays} days of diesel" divides {buyGallons} million gallons by the roughly 164 million gallons of distillate fuel the United States burns each day (3.9 million barrels). "{buyDogs} for every American" is {buyDogsTotal} billion hot dogs against a population of about 340 million. The $2.6 billion is the Pentagon's own loss estimate reported by CRS; the war's running cost, $37.5 billion, is the Defense Secretary's figure of 21 July 2026. Sony raised the US price of the PlayStation 5 from $499.99 to $549.99 on 21 August 2025, citing "a challenging economic environment"; the comparison uses the higher price.</p>
        <p style={{margin: "0", fontFamily: "'IBM Plex Mono',monospace", fontSize: "12px", letterSpacing: ".04em", color: "rgba(11,30,63,.7)", lineHeight: "1.7"}}>Sources: CRS IN12692 (aircraft and the DoD loss estimate) · Defense Secretary testimony, Senate Appropriations, 21 Jul 2026 · Sony Interactive Entertainment, US list price, 21 Aug 2025 · EIA Weekly Retail Diesel, 31 Aug 2026; EIA distillate consumption, 2024 · College Board, Trends in College Pricing 2024–25 · Costco, $1.50 hot dog and soda, unchanged since 1985 · US Census Bureau, households and population, 2024 · F-35A unit cost, Lots 15–17, F-35 Joint Program Office.</p>
      </div>
    </details>
  </section>
  ...
  <section data-screen-label="08 The world backs away" style={{position: "relative", height: "100vh", scrollSnapAlign: "start"}}>
    <div style={{position: "sticky", top: "0", height: "100vh", overflow: "hidden", background: "#0B1E3F"}}>
      <div data-cue="1" style={{position: "absolute", left: "50%", bottom: "14px", transform: "translateX(-50%)", fontFamily: "'IBM Plex Mono',monospace", fontSize: "11px", letterSpacing: ".2em", color: "rgba(247,245,240,.5)", pointerEvents: "none", opacity: "0", transition: "opacity .5s", zIndex: "2"}}>SCROLL</div>
      <canvas ref={vaultRef} style={{position: "absolute", inset: "0", width: "100%", height: "100%", display: "block"}}></canvas>
      <div className="g-top" style={{position: "absolute", top: "0", left: "0", right: "0", display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "28px 36px", pointerEvents: "none"}}>
  ...
  <section style={{background: "#F7F5F0", color: "#0B1E3F", padding: "56px 36px 72px"}}>
    <details style={{maxWidth: "820px", margin: "0 auto"}}>
      ...
      <div style={{paddingTop: "28px", display: "flex", flexDirection: "column", gap: "22px", fontSize: "17px", lineHeight: "1.5"}}>
        <p style={{margin: "0", textWrap: "pretty"}}>The Federal Reserve publishes gold held under earmark for foreign and international accounts, valued at the statutory $42.22 an ounce, unchanged since 1973. Because the price is fixed, a change in that row is a change in ounces, not in value: tonnes = millions of dollars ÷ 42.22 × 31.1035. That gives {vaultStart} tonnes in August 2025 and {vaultEnd} tonnes in June 2026, the latest provisional release: {vaultOut} tonnes out, no month with an inflow.</p>
        ...
        <p style={{margin: "0", textWrap: "pretty"}}><strong style={{fontWeight: "600"}}>The Fed's counter-argument, at full size.</strong> Colin Weiss (Federal Reserve Board, FEDS Notes, 3 September 2026): gold's apparent overtaking of Treasuries as a reserve asset is a valuation effect from private ETF demand, not central-bank buying; five legacy holders own 52% of official gold and have not added since the 1970s; foreign officials bought about $200bn of Treasuries net from 2022 to April 2026. Gold itself is down about a fifth from its January record ($5,589 to $4,471 an ounce), and the dollar index is up since the war began. Treasuries in Fed custody for foreign officials fell from $2.78tn in January to $2.62tn in June; total foreign Treasury holdings (TIC) were $9.3tn in June, the largest on record bar February.</p>
        <p style={{margin: "0", fontFamily: "'IBM Plex Mono',monospace", fontSize: "12px", letterSpacing: ".04em", color: "rgba(11,30,63,.7)", lineHeight: "1.7"}}>Sources: Federal Reserve International Summary Statistics, Table 3.13 (July 2026 release, provisional) · DNB press release 2 Sep 2026 · Bloomberg / Yahoo Finance 17 Jun 2026 · RBI half-yearly report · FEDS Notes 3 Sep 2026 · US Treasury TIC · Trading Economics for the gold price.
        </p>
      </div>
    </details>
  </section>
